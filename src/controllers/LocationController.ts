...
import * as repo from '../repositories/Cache/LocationRepositoryCache'
...
export const GetLocations = async(req:express.Request,res:express.Response) => {
    ...
    const data = await repo.GetLocations(process.env.CACHE_TYPE, connection, param)
    return res.json({ status: 200, data: data})
...
export const GetLocationById = async(req:express.Request,res:express.Response) => {
    ...
    if(id !== undefined && id !== null) {
        const locationId = parseInt(id.toString())
        const data = await repo.GetLocationById(process.env.CACHE_TYPE, connection, locationId)
        return res.json({ status: 200, data: data})
    }
    return res.status(400).json({ status: 400, data: null, error: { message: "ID_INVALID" }})
}